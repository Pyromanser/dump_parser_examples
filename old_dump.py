...
if __name__ == "__main__":
    ...
    if r.status_code != requests.codes.ok:  # Check site is online
        print("{site} status - {status_code}".format(**{'site': SITE, 'status_code': colored(str(r.status_code), 'red')}))
    else:
        print("{site} status - {status_code}".format(**{'site': SITE, 'status_code': colored(str(r.status_code), 'green')}))
        ...
        if os.path.isdir(BASE_DIR_NAME):
            ...
            for i in range(1, pages+1):
                ...
                if r.status_code != requests.codes.ok:  # Check the list page is available
                    print("{site} status - {status_code}".format(**{'site': TAG_URL, 'status_code': colored(str(r.status_code), 'red')}))
                    continue
                else:
                    print("{site} status - {status_code}".format(**{'site': TAG_URL, 'status_code': colored(str(r.status_code), 'green')}))
            ...
            for book in TRANSLATIONS:
                ...
                if r.status_code != requests.codes.ok:  # Check the list page is available
                    print("{site} status - {status_code}".format(**{'site': URL, 'status_code': colored(str(r.status_code), 'red')}))
                    continue
                else:
                    print("{site} status - {status_code}".format(**{'site': URL, 'status_code': colored(str(r.status_code), 'green')}))
